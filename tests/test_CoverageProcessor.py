...
class TestCoverageProcessor:
    ...
    def test_parse_coverage_report_json_diff(self, mocker):
        mock_parse_json_diff = mocker.patch(
            "cover_agent.CoverageProcessor.CoverageProcessor.parse_json_diff_coverage_report",
            return_value=([], [], 0.0),
        )
        processor = CoverageProcessor("fake_path", "app.py", "json_diff")
        result = processor.parse_coverage_report()
        mock_parse_json_diff.assert_called_once()
        assert result == ([], [], 0.0), "Expected result to be ([], [], 0.0)"
