...
sh tests_integration/test_with_docker.sh \
  --dockerfile "templated_tests/php_webservice/Dockerfile" \
  --source-file-path "src/public/routes.php" \
  --test-file-path "tests/AppTest.php" \
  --test-command "XDEBUG_MODE=coverage vendor/bin/phpunit --coverage-cobertura=cobertura.xml --coverage-filter=src tests/" \
